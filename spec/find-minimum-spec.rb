...
describe "find minimum" do
	...
	it "find the minimum in a maybe rotated sorted array" do
		...
		expect(find_min2([3, 1, 1])).to eq 1
		expect(find_min2([3, 1, 3])).to eq 1
		expect(find_min2([10, 1, 10, 10, 10])).to eq 1
		expect(find_min2([4, 5, 6, 6, 7, 0, 1, 1, 1, 2])).to eq 0
		expect(find_min2([4, 5, 5, 5])).to eq 4
		expect(find_min2([4])).to eq 4
	end
